...
export function createAlertRenderer() {
  return {
    blockquote({ raw, tokens }) {
      ...
      const match = stripped.match(ALERT_RE);
      if (!match) {
      ...
      const body = stripped.replace(ALERT_RE, '').trim();
      ...
      return `<div class="markdown-alert markdown-alert-${info.className}">
<p class="markdown-alert-title">${ALERT_SVGS[type]} ${info.label}</p>
${bodyParagraphs}
</div>\n`;
    },
  };
